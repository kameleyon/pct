...
- **Email notifications** (just shipped, needs your setup) — affiliate sale notification + admin order-placed notification via Resend, fired from the Stripe webhook. **Not yet functional in production** until you set `RESEND_API_KEY` (and optionally `EMAIL_FROM`, `ORDER_NOTIFICATION_EMAILS`) in Vercel and redeploy.
