...
import { getProductCountLabel } from '@/lib/catalog';
...
export async function Header() {
  const [count, session] = await Promise.all([getProductCountLabel(), getSession()]);
  return (
    <>
      {/* ===== UTILITY BAR ===== */}
      <div style={{ background: '#fff' }}>
        <div className="wrap" style={{ height: 40, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12, letterSpacing: '.01em', color: 'var(--muted)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 600 }}>
            ...
            <span style={{ color: 'var(--color-accent)', fontWeight: 600, whiteSpace: 'nowrap' }}>{count || 'Thousands of'} items</span><span className="util-hide" style={{ whiteSpace: 'nowrap' }}>in Florida local stock — same-day pickup in Zephyrhills, FL</span>
          </div>
