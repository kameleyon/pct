    ...
    <article className="prod-card" style={{ display: 'flex', flexDirection: 'column', background: '#F3EFE5', border: '1px solid rgba(43,42,38,.12)', borderRadius: 20, overflow: 'hidden', height: '100%' }}>
      <Link href={`/product/${encodeURIComponent(product.part_number)}`} style={{ display: 'flex', flexDirection: 'column', padding: 16, flex: 1, color: 'inherit' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          <span style={chip}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m7.5 4.27 9 5.15" /><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z" /><path d="M3.3 7 12 12l8.7-5" /><path d="M12 22V12" /></svg>
            Factory-direct
          </span>
          <span style={chip}>{product.measurement_system}</span>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 16px 16px' }}>
        <input defaultValue="1" aria-label="Quantity" style={{ width: 50, height: 42, textAlign: 'center', fontWeight: 600, fontSize: 15, color: 'var(--color-text)', background: '#fff', border: '1px solid rgba(43,42,38,.12)', borderRadius: 11, outline: 'none' }} />
        <button className="h-gold" style={{ flex: 1, height: 42, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer', fontWeight: 600, fontSize: 13.5, whiteSpace: 'nowrap', color: '#fff', background: 'var(--color-gold)', border: 0, borderRadius: 11 }}>
